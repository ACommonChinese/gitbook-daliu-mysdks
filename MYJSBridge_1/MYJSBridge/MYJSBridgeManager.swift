...
class MYJSBridgeManager: NSObject {
    ...
    */
    ...
    // 发送消息给js
    private func queueMessage(message: Dictionary<String, AnyObject>) {
        //TODO://if self.startupMessageQueue
        self.dispatchMessage(message)
    }
    
    // 发送消息给js
    func dispatchMessage(_ message: Dictionary<String, AnyObject>) {
        guard var messageJSON: String = self.serializeMessage(message, pretty: false) else {
            return
        }
            ...
            if let dict: Dictionary = (message as? Dictionary<String, Any>) {
                ...
                if let responseId: String = dict["responseId"] as? String {
                    ...
                    if let callbackId: String = dict["callbackId"] as? String {
                        ...
                        responseCallback = { [weak self] (responseData: AnyObject?) in
                            var message: Dictionary<String, AnyObject> = [:]
